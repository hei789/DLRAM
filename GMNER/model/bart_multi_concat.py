...
class FBartEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, image_feature, src_seq_len):
        ...
        # BERT使用1表示有效token，0表示padding（和BART输入一致）
        # BartEncoder内部会处理mask转换
        img_feat_, dict = self.bart_encoder(
            input_ids=src_tokens,
            image_feature=image_feature,
            attention_mask=mask,  # (batch, seq_len), 1=有效, 0=padding
...
class BartSeq2SeqModel(Seq2SeqModel):
    ...
    def build_model(cls, bart_model, tokenizer, label_ids, decoder_type=None,
                    use_encoder_mlp=False, box_num=36, use_bert=True, bert_model_name='bert-base-uncased', bert_model_path=None):
        # 加载BART模型（用于获取词嵌入和decoder）
        model = BartModel.from_pretrained(bart_model)
        num_tokens, _ = model.encoder.embed_tokens.weight.shape
        model.resize_token_embeddings(len(tokenizer.unique_no_split_tokens)+num_tokens)   # 扩充vocab

        # 创建使用BERT的Encoder替换原有BART Encoder
        encoder = BartEncoder(model.config, model.encoder.embed_tokens,
                              use_bert=use_bert, bert_model_name=bert_model_name, bert_model_path=bert_model_path)
