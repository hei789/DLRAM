...
class BartNERPipe(Pipe):
    def __init__(self, image_feature_path=None,
                 ...
                 image_dir=None,  # 图像文件夹路径，用于ViT在线提取
                 max_bbox=16,
                 normalize=False,
                 tokenizer='facebook/bart-base',
                 target_type='word',
                 use_vit=True,  # 是否使用ViT作为视觉编码器
                 vit_model_name='google/vit-base-patch16-224',  # ViT模型名称或本地路径
                 vit_model_path=None,  # ViT本地权重路径，如果为None则使用vit_model_name
                 device='cuda'):
        """
        :param tokenizer:
        :param dataset_name:
        ...
        :param use_vit: 是否使用ViT作为视觉编码器（在线提取）
        :param vit_model_name: ViT预训练模型名称（用于加载配置文件）
        :param vit_model_path: ViT本地权重文件夹路径（如果提供，则从本地加载）
        :param image_dir: 原始图像文件夹路径（ViT需要）
        """
        super().__init__()

        self.image_feature_path = image_feature_path  # vinvl 使用（备用）
        ...
        self.max_aspect = 6
        # ViT-B/16输出768维特征
        self.region_dim = 768 if use_vit else 2048  # ViT: 768, VinVL: 2048
        ...
        if self.use_vit:
            if vit_model_path and os.path.exists(vit_model_path):
                # 从本地加载ViT
                print(f"Loading ViT model from local path: {vit_model_path}")
                self.vit_processor = AutoImageProcessor.from_pretrained(vit_model_path)
                self.vit_model = AutoModel.from_pretrained(vit_model_path)
            else:
                # 从HuggingFace加载
                print(f"Loading ViT model from HuggingFace: {vit_model_name}")
                self.vit_processor = AutoImageProcessor.from_pretrained(vit_model_name)
                self.vit_model = AutoModel.from_pretrained(vit_model_name)
            self.vit_model.eval()
            self.vit_model.to(device)
            self.region_dim = self.vit_model.config.hidden_size
            print(f"ViT model loaded. Feature dim: {self.region_dim}")
    ...
    def extract_vit_features(self, img_id):
        """
        使用ViT在线提取图像特征
        策略：将图像划分为网格区域，每个区域用ViT的patch特征表示
        或者使用目标检测器检测到的区域（如果提供了image_feature_path）
        """
        try:
            # 加载图像
            if self.image_dir:
                ...
                if not os.path.exists(img_path):
                    img_path = os.path.join(self.image_dir, str(img_id) + '.png')
            else:
                # 尝试从image_feature_path找原图
                img_path = os.path.join(self.image_feature_path, str(img_id) + '.jpg')
            ...
            with torch.no_grad():
                ...
                # 取最后一层隐藏状态: (1, num_patches+1, hidden_dim)
                # +1是因为有CLS token
                hidden_states = outputs.last_hidden_state  # (1, 197, 768) for ViT-B/16

            # 策略：使用ViT的patch token（去掉CLS）作为区域特征
            # 14x14 = 196个patch
            patch_features = hidden_states[0, 1:, :]  # (196, 768) 去掉CLS token

            # 将196个patch映射到max_bbox个区域（通过均匀采样或聚类）
            # 简单策略：均匀采样max_bbox个patch
            num_patches = patch_features.shape[0]  # 196
            if num_patches >= self.max_bbox:
                # 均匀采样
                indices = torch.linspace(0, num_patches-1, self.max_bbox).long()
                region_features = patch_features[indices].cpu().numpy()  # (max_bbox, 768)
            else:
                # 如果patch不够，复制填充
                region_features = np.zeros((self.max_bbox, 768), dtype=np.float32)
                region_features[:num_patches] = patch_features.cpu().numpy()

            # 生成对应的伪边界框（基于patch位置）
            # ViT将224x224图像分为14x14的网格
            grid_size = 14
            patch_size = 224 // grid_size  # 16
            image_boxes = np.zeros((self.max_bbox, 4), dtype=np.float32)

            for i, patch_idx in enumerate(indices if num_patches >= self.max_bbox else range(min(num_patches, self.max_bbox))):
                row = patch_idx // grid_size
                col = patch_idx % grid_size
                # 转换为原图坐标
                xmin = col * patch_size / 224.0 * orig_w
                ymin = row * patch_size / 224.0 * orig_h
                xmax = (col + 1) * patch_size / 224.0 * orig_w
                ymax = (row + 1) * patch_size / 224.0 * orig_h
                image_boxes[i] = [xmin, ymin, xmax, ymax]

            return region_features, image_boxes, self.max_bbox

        except Exception as e:
            print(f"Error extracting ViT features for {img_id}: {e}")
            # 返回零特征
            return np.zeros((self.max_bbox, 768), dtype=np.float32), \
                   np.zeros((self.max_bbox, 4), dtype=np.float32), 0
    ...
    def process(self, data_bundle):
        ...
        def prepare_target(ins):
            ...
            if self.use_vit:
                ########### ViT在线提取特征
                image_feature_, image_boxes_, image_num = self.extract_vit_features(img_id)
                final_num = min(image_num, self.max_bbox)
                if self.normalize:
                    # L2归一化
                    norms = np.sqrt((image_feature_ ** 2).sum(axis=1, keepdims=True))
                    norms[norms == 0] = 1  # 避免除零
                    image_feature_ = image_feature_ / norms
                image_feature[:final_num] = image_feature_[:final_num]
                image_boxes[:final_num] = image_boxes_[:final_num]
            elif self.image_feature_path:
                ########### Vinvl image feature（原有代码）
                try:
