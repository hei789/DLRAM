...
class BartEncoder(nn.Module):
    """
    修改后的Encoder：使用BERT-Base作为文本编码器，ViT-B/16特征作为视觉输入
    """

    def __init__(self, config: BartConfig, embed_tokens, use_bert=True, bert_model_name='bert-base-uncased', bert_model_path=None):
        super().__init__()
        ...
        # ViT特征维度是768，与BERT-Base一致
        self.img_proj = nn.Linear(768, config.d_model)
        ...
        if use_bert:
            ...
            if bert_model_path and os.path.exists(bert_model_path):
                # 从本地加载BERT
                print(f"Loading BERT model from local path: {bert_model_path}")
                self.bert_encoder = BertModel.from_pretrained(bert_model_path)
            else:
                # 从HuggingFace加载
                print(f"Loading BERT model from HuggingFace: {bert_model_name}")
                self.bert_encoder = BertModel.from_pretrained(bert_model_name)
            # BERT输出768维，需要投影到BART的维度（768或1024）
            bert_hidden_size = self.bert_encoder.config.hidden_size  # 768
            if bert_hidden_size != config.d_model:
                self.bert_proj = nn.Linear(bert_hidden_size, config.d_model)
            else:
                self.bert_proj = None
            print(f"BERT model loaded. Hidden size: {bert_hidden_size}")
        else:
            # 使用原始的Transformer Encoder
    ...
    def forward(
        ...
        Args:
            ...
            image_feature (FloatTensor): ViT features of shape `(batch, box_num, 768)`
            attention_mask (torch.LongTensor): indicating which indices are padding tokens
        Returns:
            BaseModelOutput or Tuple comprised of:
                - **x** (Tensor): the last encoder layer's output
                - **encoder_states** (tuple): all intermediate hidden states
                - **all_attentions** (tuple): Attention weights for each layer
        """
        # BERT/BART使用不同的attention mask格式
        # BERT: 1表示有效token, 0表示padding
        # BART内部: 0表示有效token, 1表示padding (通过invert_mask转换)

        # 保存原始attention_mask用于BERT
        bert_attention_mask = attention_mask

        # BART格式的mask转换（用于后续输出）
        if attention_mask is not None:
            attention_mask = invert_mask(attention_mask)

        # 文本嵌入（BART的embedding方式）
        inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
        embed_pos = self.embed_positions(input_ids)
        text_embeds = inputs_embeds + embed_pos
        text_embeds = self.layernorm_embedding(text_embeds)
        text_embeds = F.dropout(text_embeds, p=self.dropout, training=self.training)

        if self.use_bert:
            # 使用BERT编码文本
            # BERT输入: (batch, seq_len, hidden)
            bert_outputs = self.bert_encoder(
                inputs_embeds=text_embeds,
                attention_mask=bert_attention_mask,  # BERT格式: 1=有效, 0=padding
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_dict=True
            )
            # BERT输出: (batch, seq_len, 768)
            ...
            # 获取encoder states用于decoder的copy机制
            encoder_states = None
            if output_hidden_states:
                # 包含embedding层输出
                encoder_states = bert_outputs.hidden_states
        else:
            # 使用原始Transformer Encoder
            x = text_embeds.transpose(0, 1)  # (seq_len, batch, hidden)
