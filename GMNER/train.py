...
parser.add_argument('--image_dir',default='./Twitter_GMNER/images/', type=str, help='原始图像文件夹路径(ViT需要)')
...
# 新增编码器相关参数
parser.add_argument('--use_bert', default=True, type=bool, help='是否使用BERT作为文本编码器')
parser.add_argument('--bert_model_name', default='bert-base-uncased', type=str, help='BERT模型名称（当bert_model_path为None时使用）')
parser.add_argument('--bert_model_path', default=None, type=str, help='BERT本地权重文件夹路径，如果提供则从本地加载')
parser.add_argument('--use_vit', default=True, type=bool, help='是否使用ViT作为视觉编码器')
parser.add_argument('--vit_model_name', default='google/vit-base-patch16-224', type=str, help='ViT模型名称（当vit_model_path为None时使用）')
parser.add_argument('--vit_model_path', default=None, type=str, help='ViT本地权重文件夹路径，如果提供则从本地加载')
...
def get_data():
    pipe = BartNERPipe(
        ...
        image_dir=args.image_dir,  # ViT需要原始图像
        ...
        vit_model_path=args.vit_model_path,  # ViT本地路径
        device=device
    )
...
model = BartSeq2SeqModel.build_model(
    args.bart_name, tokenizer, label_ids=label_ids, decoder_type=args.decoder_type,
    use_encoder_mlp=args.use_encoder_mlp, box_num=args.box_num,
    use_bert=args.use_bert, bert_model_name=args.bert_model_name, bert_model_path=args.bert_model_path)
